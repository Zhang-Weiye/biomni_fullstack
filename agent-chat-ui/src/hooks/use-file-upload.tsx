...
export const SUPPORTED_FILE_TYPES = [
  ...
  "application/json",
  "application/x-hdf5",
  "application/octet-stream", // 用于 .h5ad 等二进制文件
...
export function useFileUpload({
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const duplicateFiles = allFiles.filter((file) =>
  ...
  const handlePaste = async (
    ...
    const duplicateFiles = allFiles.filter(isDuplicate);
    const uniqueFiles = allFiles.filter((file) => !isDuplicate(file));
