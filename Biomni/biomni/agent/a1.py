...
from typing_extensions import Any, Literal, TypedDict, Optional, Annotated, Sequence
...
class A1:
    def __init__(
        ...
        self.llm = get_llm(
            ...
            base_url=base_url,
            api_key=api_key,
            verify_ssl=verify_ssl  # 传递SSL验证参数
        )
  ...
  - Bash scripts and commands: Use the #!BASH marker in your execute block for both simple commands and complex shell scripts with variables, loops, conditionals, etc.
    ...
    def configure(self, self_critic=False, test_time_scale_round=0):
        ...
        def generate(state: AgentState) -> AgentState:
            ...
            # 返回消息而不是直接修改 state
            if answer_match:
                return {"messages": [AIMessage(content=msg.strip())], "next_step": "end"}
            elif execute_match:
                return {"messages": [AIMessage(content=msg.strip())], "next_step": "execute"}
            elif think_match:
                return {"messages": [AIMessage(content=msg.strip())], "next_step": "generate"}
            else:
                # 处理解析错误的情况
                error_count = sum(
                ...
                if error_count >= 2:
                    return {"messages": [AIMessage(content="Execution terminated due to repeated parsing errors.")], "next_step": "end"}
                else:
                    return {
                        "messages": [
                            AIMessage(content=msg.strip()),
                            HumanMessage(content="Each response must include thinking process followed by either <execute> or <solution> tag. But there are no tags in the current response. Please follow the instruction, fix and regenerate the response again.")
                        ], 
                        "next_step": "generate"
                    }
        ...
        def execute(state: AgentState) -> AgentState:
            ...
            if execute_match:
                ...
                # Process images in the result before adding to context
                result = self._process_images_in_result(result)
        ...
        # 因为langgraph的checkpointer在workflow.compile()时会自动创建，所以这里不需要再创建
    ...
    def _process_images_in_result(self, result: str) -> str:
        """Process images in the execution result and save them automatically."""
        import os
        import base64
        from datetime import datetime
        
        # Get the saved_pictures_dir from config
        try:
            from ..config import default_config
            saved_pictures_dir = default_config.saved_pictures_dir
        except ImportError:
            saved_pictures_dir = "./saved_pictures"
        
        # Ensure directory exists
        os.makedirs(saved_pictures_dir, exist_ok=True)
        
        # Always check for matplotlib figures after code execution
        # This ensures we capture figures even when plt.show() doesn't produce text output
        try:
            import matplotlib.pyplot as plt
            import matplotlib
            
            # --- Redirect all matplotlib savefig calls to saved_pictures_dir ---
            # This avoids scattering images into ad-hoc folders like "visualizations".
            try:
                # Only patch once per process
                if not getattr(matplotlib, "_biomni_savedir_patched", False):
                    from matplotlib.figure import Figure

                    _original_fig_savefig = Figure.savefig

                    def _patched_fig_savefig(self, fname, *args, **kwargs):  # type: ignore[override]
                        import os as _os
                        # If a path is provided, rewrite it to saved_pictures_dir while keeping the basename
                        if isinstance(fname, str):
                            fname = _os.path.join(saved_pictures_dir, _os.path.basename(fname))
                        return _original_fig_savefig(self, fname, *args, **kwargs)

                    Figure.savefig = _patched_fig_savefig  # type: ignore[assignment]
                    matplotlib._biomni_savedir_patched = True
            except Exception:
                # If patching fails, continue without interrupting flow
                pass
            ...
            if fig_nums:
                ...
                for fig_num in fig_nums:
                    ...
                    if fig.get_axes():  # Only save if figure has content
                        ...
                        filename = f"auto_saved_figure_{fig_num}_{timestamp}.png"
                        filepath = os.path.join(saved_pictures_dir, filename)
                        
                        # Save the figure
                        fig.savefig(filepath, dpi=150, bbox_inches='tight', 
                                   facecolor='white', edgecolor='none')
                        
                        # Add picture showing message to result with API URL
                        filename = os.path.basename(filepath)
                        api_url = f"/api/images/{filename}"
                        result += f"\n📷 picture showing: {api_url}"
                        
                        # Close the figure to free memory
                        plt.close(fig)
            else:
                # If no figures found, try to get current figure
                # This handles cases where plt.show() was called but figures might not be in get_fignums()
                try:
                    current_fig = plt.gcf()
                    if current_fig.get_axes():
                        # Generate filename
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        filename = f"auto_saved_current_figure_{timestamp}.png"
                        filepath = os.path.join(saved_pictures_dir, filename)
                    ...
                    # If current figure approach fails, continue without error
                    pass
                        
        except ImportError:
            # matplotlib not available, skip processing
            pass
        except Exception as e:
            # If there's an error, just continue without processing
            pass
